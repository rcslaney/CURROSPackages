...
def full_reset_and_calibrate_all():
    ...
    for drive_name, drive in odrives.items():
        ...
        for axis_id in range(2):
            ...
            set_axis_settings(axis)
            calibrate(drive_name, axis, axis_id)

            drive.save_configuration()
            axis.requested_state = AXIS_STATE_CLOSED_LOOP_CONTROL
            print("Odrive " + drive_name + "(" + drive_cfg["SERIAL_NO"] + ") ended up in state " + AXIS_STATE(axis.current_state))
    print("Calibrations complete")
    set_all_limits()


def reboot(drive_name, drive):
    drive_cfg = config.ODRIVES[drive_name]
...
def calibrate(drive_name, axis, axis_id="?"):
    drive_cfg = config.ODRIVES[drive_name]
...
def add_axis_distance(axis, dis):
    pos = dis * config.DRIVE["DRIVE_GEARING"] / (2 * math.pi * config.DRIVE["WHEEL_RADIUS"]) * config.DRIVE["CPR"]
    pos = add_axis_position(axis, pos)
    return pos / config.DRIVE["DRIVE_GEARING"] * (2 * math.pi * config.DRIVE["WHEEL_RADIUS"]) / config.DRIVE["CPR"]


def set_axis_rps(axis, rps):
    axis.controller.config.control_mode = CTRL_MODE_VELOCITY_CONTROL
    axis.controller.vel_setpoint = rps * config.DRIVE["CPR"]
...
def drive_velocity(speed, angular_speed):
    ...
    set_axis_drive_velocity(axes[config.ODRIVES["DRIVE"]["RIGHT"]["AXIS"]], speed_r * config.ODRIVES["DRIVE"]["RIGHT"]["DIRECTION"])
...
def set_all_limits():
    pass  # set_vel_limits(config.DRIVE["MAX_DRIVE_SPEED"], config.DRIVE["MAX_FLIPPER_SPEED"])  # set_acc_limits(1)  # set_curr_limits(config.DRIVE["MAX_CURRENT"], config.DRIVE["MAX_CURRENT"])
...
def axis_states():
    ...
    for name, axis in axes.items():
        axis_info.append([name, AXIS_STATE(axis.current_state), CTRL_MODE(axis.controller.config.control_mode), axis.controller.pos_setpoint, axis.motor.config.current_lim,
            axis.controller.current_setpoint, axis.controller.config.vel_limit, hex(axis.error), hex(axis.motor.error),
            hex(axis.controller.error) if hasattr(axis.controller, "error") else hex(0)])
